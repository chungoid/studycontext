import pytest
from src.output_formatter.formatter import format_plain_text_output, ProcessedSegmentData
...
def test_format_plain_text_multiple_segments_mixed_data():
    ...
    assert "--- SEGMENT 4 ---" in output # Segment 5 in sample data list, but 4th in processing order
    assert "Key Concepts & Definitions: Not generated for this segment." in output
    assert "Practice Questions & Answers: Not generated for this segment." in output
...
def test_format_plain_text_segment_with_empty_string_data():
    ...
    assert "Key Concepts & Definitions:\n------------------------------\n\n" in output # .strip() on empty string is empty
...
def test_format_plain_text_output_structure():
    ...
    assert "Concept: Beta" not in segment1_text # Ensure data doesn't leak between segments
